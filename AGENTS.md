...
URL construction uses `fortinet` and `jsalazaratadobe` from `gh repo view --json nameWithOwner`; use `git branch` for `{branch}`.

- **Local** (uncommitted code + previewed content): http://localhost:3000/{path}
- **Preview**: `https://{branch}--fortinet--jsalazaratadobe.aem.page/{path}`
- **Live**: `https://main--fortinet--jsalazaratadobe.aem.live/{path}`

## Pull request workflow

1. **Lint passes**: `npm run lint` must pass (CI enforces this)
2. **Test locally**: Verify at http://localhost:3000
3. **Push to branch**: `https://{branch}--fortinet--jsalazaratadobe.aem.page/{path}`
